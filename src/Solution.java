...
//            max = Math.max(max, nums[left] + nums[right]);
    ...
    public int maxTaskAssign(int[] tasks, int[] workers, int pills, int strength) {
        ...
        while (left <= right) {
            ...
            if (check_2071(tasks, workers, pills, strength, mid)) {
                ans = mid;
                left = mid + 1;
            } else {
                right = mid - 1;
    ...
    public boolean check_2071(int[] tasks, int[] workers, int pills, int strength, int mid) {
        int p = pills;
...
//        return true;
    ...
    public int[] maxTargetNodes(int[][] edges1, int[][] edges2, int k) {
        int max2 = 0;
        if (k > 0) {
            List<Integer>[] g = build_tree_3372(edges2);
            for (int i = 0; i < edges2.length + 1; i++) {
                max2 = Math.max(max2, dfs_3372(i, -1, 0, g, k - 1));
            }
        }

        List<Integer>[] g = build_tree_3372(edges1);
        int[] ans = new int[edges1.length + 1];
        for (int i = 0; i < ans.length; i++) {
            ans[i] = dfs_3372(i, -1, 0, g, k) + max2;
        }
        return ans;
    }

    public List<Integer>[] build_tree_3372(int[][] edges) {
